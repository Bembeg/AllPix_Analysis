...
def InterpolateHist(hist, x):
        #find left bin edge  
#        print("searching for value of x:",x) 
        xBin = hist.FindBin(x)
        binWidth = hist.GetBinWidth(hist.GetBin(xBin))
        for i in range(10):
                currBin = hist.FindBin(x-i*binWidth) 
                #print("bin:",currBin,"cont:",hist.GetBinContent(currBin))
                if hist.GetBinContent(currBin) > 0.01:
                        if i == 0:
                 #               print("I already am at the right bin, no need to interpolate, just take the x,y")
